...
static func load_csv(path: String, as_dictionary := true):
	...
	if not path.is_absolute_path() or not file_exists:
		push_error("FileManager: Failed to load CSV because file doesn't exist. Path: %s", path)
	...
	if open_error != Error.OK:
		push_error("FileManager: ERROR_CODE[%s] Error opening file %s" % [open_error,path])
		return ERR_PARSE_ERROR
	...
	if as_dictionary and lines.size() > 1:
		...
		for i in range(1, lines.size()):
			...
			if current_fields.size() > column_headers.size():
				push_error("FileManager: Reading the file %s it was detected a line that has more fields than column headers [] < []" % [path, column_headers.join(","), current_fields.join(",")])
				return ERR_PARSE_ERROR
