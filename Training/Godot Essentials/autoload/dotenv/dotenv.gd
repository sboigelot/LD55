...
@export var ENVIRONMENT_FILES_PATH: String = _default_path()
...
func create_environment_file(filename: String = ".env", overwrite: bool = false) -> void:
	if overwrite or not _env_file_exists(filename):
		...
		if error:
			push_error("dotenv: {error}".format({"error": error}))
			return
...
func flush_environment_variables(_filename: String = ".env", except: Array[String] = []) -> void:	
...
func _read_file_with_callback(filename: String = ".env", callback: Callable = func(_line): pass):
