...
@export var rotation_effect := ROTATION_EFFECTS.SMOOTHLY
...
func _unhandled_input(_event: InputEvent):
	if InputWizard.action_just_pressed_and_exists(input_action_rotate_left):
		rotate_target(Vector2.LEFT)
		
	if InputWizard.action_just_pressed_and_exists(input_action_rotate_right):
		rotate_target(Vector2.RIGHT)


func rotate_target(direction: Vector2):
	match rotation_effect:
		ROTATION_EFFECTS.SMOOTHLY:
			rotate_with_smoothly_effect(direction)
		ROTATION_EFFECTS.COMPRESSED:
