@icon("res://components/movement/3D/input/dungeon_grid_movement.svg")
...
func _can_move(direction: Vector2) -> bool:
	return target and (tween_movement == null or (tween_movement and not tween_movement.is_running()))


func _can_rotate(direction: Vector2) -> bool:
	return target and (tween_movement == null or (tween_movement and not tween_movement.is_running())) and direction in [Vector2.LEFT, Vector2.RIGHT]
