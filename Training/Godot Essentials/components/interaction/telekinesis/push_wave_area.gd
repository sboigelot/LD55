...
func push_bodies_on_range():
	for body: RigidBody3D in get_overlapping_bodies().filter(func(detected_body): return detected_body is RigidBody3D and not bodies_pushed.has(detected_body.name)):
		...
		body.angular_velocity = VectorWizard.generate_3d_random_fixed_direction() * rng.randf_range(0.5, 1.0)
		body.apply_impulse(VectorWizard.rotate_horizontal_random(direction) * rng.randf_range(min_push_force, max_push_force), -body.position.normalized() + upward_offset)
