...
var sound_dictionary = {
	SOUND_GROUP.BIP : ["bip0", "bip1"], 
	SOUND_GROUP.DING : ["ding"], 
}

func play_from_group(sound_ground:int):
	var sound = get_rand_sound_from_group(sound_ground)
	if sound == "":
		return
		
	play(sound)
	
func get_rand_sound_from_group(sound_ground:int)->String:
	if not sound_dictionary.has(sound_ground):
		return ""
		
	var group:Array = sound_dictionary[sound_ground]
	if group.size() == 1:
		return group[0]
	var select_sound = group[randi() % group.size()]
	return select_sound
